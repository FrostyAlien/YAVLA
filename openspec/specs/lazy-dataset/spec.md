# lazy-dataset Specification

## Purpose
Define the `LazyLeRobotDataset` backend for memory-efficient, random-access loading of LeRobot v3 datasets via
metadata-driven initialization, shard-index-based lookup, on-demand Parquet row reads, temporal context
(`delta_timestamps`), action chunking, and video decoding.
## Requirements
