...
**Critical architectural insight:** PaliGemma is decoder-only (SigLIP → linear projector → Gemma LM). Its `forward()` accepts `inputs_embeds` to bypass the built-in vision pipeline. `token_type_ids` controls bidirectional (image, type=0 prefix) vs causal (text, type=1) attention — but ONLY when `labels` is provided (triggers `is_training=True` path). Without labels, all tokens get full bidirectional attention. Pass dummy `labels` (all `-100`) to enforce the hybrid causal mask. [modeling_paligemma.py#L304-L387](https://github.com/huggingface/transformers/blob/556312cd/src/transformers/models/paligemma/modeling_paligemma.py#L304-L387)
