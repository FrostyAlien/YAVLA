...
- **WHEN** `ImageTransform(transforms=[Resize([224, 224], 3), Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))], camera_keys=["image"])` is applied to a sample with `image` as a float tensor
- **THEN** the output `image` tensor SHALL be resized to `[3, 224, 224]`, have dtype `float32`, and be normalized approximately to `[-1, 1]`
...
The transform system SHALL support config-driven torchvision v2 transforms sufficient to express SigLIP-224 preprocessing via string specs.
...
#### Scenario: Canonical SigLIP-224 transform list builds successfully
- **WHEN** `build_torchvision_transforms(["Resize([224, 224], 3)", "Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))"])` is called
