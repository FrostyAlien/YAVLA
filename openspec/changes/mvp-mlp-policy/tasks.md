...
- [ ] 1.1 Create `src/yavla/models/` package with `__init__.py`, `types.py`
- [ ] 1.2 Implement `ObservationBatch`, `TokenBatch` (with `token_type_ids`), `BackboneOutput`, `ActionPrediction`, `ActionChunk`, `LossDict`, `TrainingBatch` dataclasses in `types.py`
...
- [ ] 1.3 Implement `ActionSpaceSpec` and `ProprioSpec` dataclasses in `types.py`
- [ ] 1.4 Implement `FreezeConfig` dataclass in `types.py` with `freeze_modules: list[str]`, `lora_target_modules: list[str]` (peft leaf module names e.g. `["q_proj", "v_proj"]`, NOT full paths), `lora_r: int = 8`, `lora_alpha: int = 16`, `lora_dropout: float = 0.0`
...
- [ ] 2.1 Implement `Registry[ConfigT, ModuleT]` generic class in `src/yavla/models/registry.py` with `register()` decorator, `build()`, `list()`, `get_default_config()`
...
- [ ] 3.3 Implement ABC base classes: `VisionEncoderBase`, `BackboneBase`, `ActionHeadBase`, `ActionDecoderBase`, `ProprioEncoderBase`
...
- [ ] 3.4 Implement `validate_integration(backbone, head)` function with `IncompatibleError`
...
- [ ] 4b.1 Implement `ProprioEncoderConfig` dataclass in `src/yavla/models/encoders/proprio.py`
- [ ] 4b.2 Implement `ProprioEncoder(ProprioEncoderBase)` — single `nn.Linear(proprio_dim, backbone_dim)` projecting `[B, D_proprio]` → `[B, 1, D_backbone]`
...
- [ ] 4b.3 Register `ProprioEncoder` with `proprio_registry`
...
- [ ] 6.3 Register `VLMBackbone` with `backbone_registry`

## 7. Action Head

- [ ] 7.1 Implement `MLPHeadConfig` dataclass in `src/yavla/models/heads/mlp.py`
- [ ] 7.2 Implement `MLPResNet` module (LayerNorm → Linear → ReLU → residual blocks → output)
- [ ] 7.3 Implement `MLPRegressionHead(ActionHeadBase)` — mean-pool `readout_states [B, N_readout, D] → [B, D]`, pass through MLPResNet, predict action chunks, L1 loss
...
- [ ] 7.4 Register `MLPRegressionHead` with `head_registry`
...
- [ ] 8.4 Implement `build_policy(PolicyConfig) → VLAPolicy` factory:
  ...
  <!-- Ref: peft.get_peft_model + LoraConfig: https://huggingface.co/docs/peft/main/en/package_reference/lora#peft.LoraConfig
       Ref: OpenVLA-OFT freeze+LoRA: https://github.com/moojink/openvla-oft/blob/e4287e94/prismatic/training/train.py#L89-L120 -->
- [ ] 8.5 Implement `save_pretrained` / `from_pretrained` — safetensors + config.json + action_stats.json + embodiment.json. For LoRA models, use `peft` adapter save/load (`model.save_pretrained` / `PeftModel.from_pretrained`)

## 9. Wiring and Exports

- [ ] 9.1 Wire all registry imports in `src/yavla/models/__init__.py` so registrations execute on import
- [ ] 9.2 Export public API: `build_policy`, `VLAPolicy`, `PolicyConfig`, all Protocol types, all data containers
